...
namespace rirc
{
		const str_t kMessagePrefix("IRCMessagePrefix");
		const str_t kMessageCommand("IRCMessageCommand");
		const str_t kMessageArges("IRCMessageArges");
		const str_t kMessageTail("IRCMessageTail");
		const str_t kMessageSource("IRCMessageSource");

		Message::Message(const str_t& msg,
						 const str_t & command, 
						 const str_t & prefix, 
		...
		{
			static dict_t* p_ircFlags = NULL;
			if (NULL == p_ircFlags) {
				p_ircFlags = new dict_t();
				for (int i = 0; i < 500; ++i) {
					p_ircFlags->insert(std::pair<str_t,str_t>(ircflags[i][0],ircflags[i][1]));
				}
